...
pub fn sha256(input: &[u8]) -> [u8; DIGEST_LEN] {
    ...
    // Pad: append 0x80, then zeros up to 56 mod 64, then the 64-bit big-endian
    // bit length.
    ...
    for chunk in msg.chunks_exact(BLOCK_LEN) {
        // Prepare the 64-entry message schedule (big-endian words).
        let mut w = [0u32; 64];
...
/// The 32-byte HMAC-SHA-256 of `message` under `key` (RFC 2104).
///
/// A key longer than the 64-byte block is first hashed; a shorter key is
/// zero-padded to the block. The result is
/// `SHA256((key ^ opad) || SHA256((key ^ ipad) || message))`.
...
mod tests {
    ...
    #[test]
    fn sha256_matches_the_fips_example_vectors() {
        ...
        // A two-block message (exercises the message schedule across chunks).
        assert_eq!(
            hex(&sha256(
    ...
    fn hmac_sha256_matches_rfc4231_vectors() {
        ...
        // RFC 4231 Test Case 6: a 131-byte key (> block size ⇒ key is hashed first).
        assert_eq!(
            hex(&hmac_sha256(
                &[0xaa; 131],
