...
/// The sine-derived constants `floor(2^32 · abs(sin(i + 1)))` (RFC 1321 §3.4).
const K: [u32; 64] = [
...
/// The 16-byte MD5 digest of `input` (RFC 1321).
pub fn md5(input: &[u8]) -> [u8; 16] {
    let (mut a0, mut b0, mut c0, mut d0) =
        (0x6745_2301u32, 0xefcd_ab89u32, 0x98ba_dcfeu32, 0x1032_5476u32);

    // Pad: append 0x80, then zeros up to 56 mod 64, then the 64-bit little-endian
    // bit length.
...
mod tests {
    ...
    fn rfc1321_test_vectors() {
        // From RFC 1321, Appendix A.5. The last two vectors exceed 55 bytes, so they
        // exercise the multi-block path (padding spilling into a second 64-byte block).
