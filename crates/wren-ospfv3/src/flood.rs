//! The flooding procedure (RFC 5340 §4.5, inheriting RFC 2328 §13) — what to do
//! with an LSA that arrived in a Link State Update, and where to send LSAs onward.
//!
//! RFC 5340 leaves the flooding *decision* unchanged from OSPFv2: it turns on the
//! §13.1 recency test of the LSA header, which is identical in OSPFv3. So this is
//! a near-verbatim port of `wren_ospf::flood`. [`decide_flood`] takes the
//! link-state database for the LSA's scope, the received LSA and the few facts the
//! runner knows about the receiving adjacency, and returns a [`FloodDecision`] —
//! install and reflood, acknowledge, send our newer copy back, or discard. It does
//! *not* mutate the database: the runner installs on [`FloodDecision::Install`].
//! [`flood_scope`] is the companion §13.3 rule deciding the multicast destination
//! for sending an LSA out a given interface — the part that makes a DROther flood
//! only to the DR/BDR.
//!
//! Steps 1–3 of §13 (checksum, known LS type, scope filtering) happen earlier — the
//! LSA reached here already decoded and checksum-verified by [`crate::packet`] /
//! [`crate::lsa`]; this module is §13 steps 4 onward. One OSPFv3 nuance lives a
//! layer up, not here: the *scope* of an LSA (link-local / area / AS, from its LS
//! Type) selects which database the runner passes in and how far a reflood reaches.
...
pub struct FloodInput<'a> {
    /// The link-state database for the LSA's scope (the interface's link-local
    /// database for a Link-LSA, the area's for area-scoped types, or the AS-wide
    /// database for an AS-external LSA).
    pub lsdb: &'a Lsdb,
...
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum FloodScope {
    /// `AllSPFRouters` (`ff02::5`) — every router on the link.
    AllSpfRouters,
    /// `AllDRouters` (`ff02::6`) — only the DR and BDR.
    AllDRouters,
}
...
mod tests {
    use super::*;
    use crate::lsa::{Lsa, LsType, LsaBody, LsaHeader, RouterLsa};
