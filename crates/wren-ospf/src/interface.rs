//! The interface state machine (RFC 2328 §9) and the Designated Router election
//! (§9.4) — the per-interface logic that decides this router's role on a network
//! and, on multi-access links, who the DR and BDR are.
//!
//! Both pieces are pure. [`elect_dr_bdr`] is the §9.4 algorithm as a free
//! function over a candidate set (each router's priority and its currently
//! advertised DR/BDR), including the §9.4(5) recomputation when the calculating
//! router's own role flips. [`Interface::handle`] is the §9.3 transition table;
//! on the events that trigger an election it folds the result back into the
//! interface state (DR / Backup / DROther). The runner supplies events and the
//! 2-Way-or-better neighbour set; no sockets or timers live here.
//!
//! Identities are modelled as Router IDs throughout (the election's tie-break and
//! the `0.0.0.0 = none` sentinel). A real OSPFv2 speaker maps a Hello's DR/BDR
//! interface-address fields to neighbour Router IDs via its neighbour table
//! before electing; that mapping is the runner's job.
...
impl Interface {
    ...
    /// Whether this router is the DR or BDR on this interface — the §10.4 test
    /// for whether to form full adjacencies with DROther neighbours.
...
mod tests {
    ...
    fn lone_router_becomes_dr_with_no_backup() {
        // A single eligible router on a fresh segment: DR=self, BDR=none.
    ...
    fn steady_state_two_routers() {
        // Converged declarations: R2 says DR=R2, R1 says BDR=R1. From R1's view.
    ...
    fn priority_beats_router_id() {
        // R1 has the lower ID but the higher priority and claims DR; it wins.
    ...
    fn priority_zero_router_is_never_dr_or_bdr() {
        // R1 is ineligible (priority 0); only R2 can hold a role.
    ...
    fn existing_dr_is_not_preempted_by_a_higher_priority_newcomer() {
        // R2 (prio 1) is the established DR; R9 (prio 100) joins declaring nothing.
        // §9.4(4) keeps R2 as DR (it declares itself); R9 only becomes BDR.
    ...
    fn broadcast_interface_up_waits_then_elects_dr() {
        ...
        // Wait timer fires with no neighbours → this router is the DR.
    ...
    fn neighbor_change_demotes_to_backup_when_a_higher_router_appears() {
        // We came up alone and became DR.
        let mut iface = Interface::new(ip([1, 1, 1, 1]), 1, InterfaceType::Broadcast);
        iface.handle(InterfaceUp, &[]);
        iface.handle(WaitTimer, &[]);
        assert_eq!(iface.state, Dr);

        // R2 (higher ID) appears already declaring itself DR (it was DR elsewhere).
        let r2 = cand([2, 2, 2, 2], 1, [2, 2, 2, 2], NONE);
    ...
    fn idempotent_neighbor_change_reports_no_change() {
        ...
        iface.handle(InterfaceUp, &[]);
        iface.handle(WaitTimer, &[]);
        // Re-running with the same (empty) neighbour set elects the same DR.
