//! The neighbour state machine (RFC 2328 §10) — the per-neighbour conversation
//! that takes a router from "just heard a Hello" all the way to a fully
//! synchronised adjacency.
//!
...
//! the runner must carry out (arm timers, start the Database Description
//! exchange, signal the adjacency up/down). No sockets, no clock: the runner
//! supplies events and executes actions, so every transition in §10.3 is
//! unit-testable.
//!
//! The neighbour also caches the priority and DR/BDR its Hellos advertise, which
//! is exactly what the interface's [`crate::interface`] DR election consumes.
...
/// A neighbour as tracked by one interface (§10.1): its identity, the state of
/// the conversation, and the priority/DR/BDR its Hellos advertise (cached here
/// for the interface's DR election in [`crate::interface`]).
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct Neighbor {
...
mod tests {
    ...
    fn full_adjacency_bringup_path() {
        ...
        // Request list empty → straight to Full.
    ...
    fn two_way_without_adjacency_stays_two_way() {
        ...
        // Later the election makes us DR → AdjOK reconsiders and forms it.
    ...
    fn one_way_drops_back_to_init() {
        ...
        // From plain TwoWay it also returns to Init, with no adjacency to clear.
    ...
    fn inactivity_and_kill_return_to_down() {
        ...
        let mut m = Neighbor::new(ip([3, 3, 3, 3]));
        m.state = TwoWay;
        // No adjacency was forming, so nothing to clear — just stop the timer.
