#!/usr/bin/env bash
# BGP Extended Next Hop Encoding (RFC 5549 / RFC 8950) smoke test — advertise an
# IPv4 route with an IPv6 next hop, and install it in the kernel via that IPv6
# gateway (RTA_VIA). Self-contained, rootless.
#
...
# (CAP_NET_BIND_SERVICE, held by the netns-root inside `unshare -Urn`); installing
# an IPv4-via-IPv6 route needs CAP_NET_ADMIN, likewise held there. Per-daemon
# control sockets are Unix sockets under a temp dir, not the network.
#
# Topology: A (AS 65001) <-eBGP-> B (AS 65002, passive), over a dual-stack veth.
# The link carries BOTH IPv4 (10.0.0.0/24, for the BGP TCP session) AND IPv6
# (2001:db8::/64, the advertised next hop). B originates the IPv4 prefix
# 10.99.0.0/24 and, with extended-nexthop negotiated, advertises it in
# MP_REACH_NLRI (AFI IPv4) with its IPv6 next-hop-self. On a directly-connected
# link the speaker sends a 32-octet next hop (global + link-local, RFC 2545), and
# the receiver forwards over the link-local pinned to the ingress interface — so A
# installs the prefix via B's link-local next hop:
#
...
#
# Usage:  bash scripts/bgp-extended-nexthop-smoke.sh
set -euo pipefail
...
# A — eBGP, extended-nexthop on, installs received IPv4-via-IPv6 routes.
cat >"$WORK/a.toml" <<EOF
router-id = "10.0.0.1"
[bgp]
enabled  = true
...
# B — originates 10.99.0.0/24, advertises it with its IPv6 next-hop-self.
cat >"$WORK/b.toml" <<EOF
router-id = "10.0.0.2"
[bgp]
enabled   = true
...
unshare -Urn bash -c '
  ...
  # Dual-stack the link: IPv4 for the BGP session, IPv6 for the advertised next hop.
  ...
  # Let IPv6 DAD settle so 2001:db8::2 is usable as a gateway.
  sleep 2
...
ok=1
# A learned the IPv4 prefix with an IPv6 next hop in its BGP RIB (the link-local
# next hop the directly-connected speaker advertised, RFC 2545 + RFC 5549).
...
if [[ $ok -ne 1 ]]; then echo "--- A log ---"; tail -15 "$WORK/a.log"; echo "--- B log ---"; tail -15 "$WORK/b.log"; fi
[[ $ok -eq 1 ]] || exit 1
echo "bgp extended-nexthop (RFC 5549) smoke test: OK"
